...
func (app *App) UploadMUSICDETAILS(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectMUSICDETAILS(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileMUSICDETAILS(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectMUSICDETAILS(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentMUSICDETAILS(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectMUSICDETAILS(parent *Internals, user *User, n int, name string, b []byte) (*MUSICDETAILS, error) {
	fields := FieldsMUSICDETAILS{}
	object := user.NewMUSICDETAILS(parent, fields)
	...
	img, err := object.ValidateImageMUSICDETAILS(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeMusicdetailsFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeMusicdetailsFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}
...
func readZipFileMUSICDETAILS(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeMusicdetailsFile(bucketName, objectName string, content []byte) error {
