...
// api-musicdetails
func (app *App) EntrypointMUSICDETAILS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addMusicdetailsAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeMusicdetailsAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				{
					Path: "fields.albumname",
					Value: object.Fields.Albumname,
				},{
					Path: "fields.trackname",
					Value: object.Fields.Trackname,
				},{
					Path: "fields.albumimage",
					Value: object.Fields.Albumimage,
				},{
					Path: "fields.trackimage",
					Value: object.Fields.Trackimage,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getMusicdetailsList(subject *MUSICDETAILS) []*MUSICDETAILS {
	list := []*MUSICDETAILS{}
