...
// api-things
func (app *App) EntrypointTHINGS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "prompt":
				...
				reply, err := app.thingChatGPTPrompt(user, object, prompt)
				if err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "create":
				...
				if err := app.thingChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.thingChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "vertex":
			...
			case "create":
				...
				if err := app.thingVertexCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "init":
			...
			if err := app.CreateDocumentTHING(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadTHING(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of things
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "things"),
			}
		...
		case "list":
			...
			app.thingLists(w, user, parent, mode, limit)
