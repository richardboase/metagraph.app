...
// api-animals
func (app *App) EntrypointANIMALS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := FieldsANIMAL{}
			object := user.NewANIMAL(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentANIMAL(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of animals
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "animals"),
			}
		...
		case "list":
			...
			app.animalLists(w, user, nil, mode, limit)
