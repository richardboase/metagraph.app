...
func (app *App) UploadDONATION(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectDONATION(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileDONATION(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectDONATION(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentDONATION(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectDONATION(parent *Internals, user *User, n int, name string, b []byte) (*DONATION, error) {
	fields := FieldsDONATION{}
	object := user.NewDONATION(parent, fields)
	...
	img, err := object.ValidateImageDONATION(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeDonationFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeDonationFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}

	object.Meta.Media.Preview = "https://storage.googleapis.com/rescue-center-management-uploads/" + uri

	return object, nil
}

func readZipFileDONATION(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeDonationFile(bucketName, objectName string, content []byte) error {
