...
func (app *App) floorLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("floors")
	} else {
		collection = app.Firestore().Collection("floors")
	}
