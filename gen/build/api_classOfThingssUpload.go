...
func (app *App) UploadCLASSOFTHINGS(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectCLASSOFTHINGS(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileCLASSOFTHINGS(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectCLASSOFTHINGS(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentCLASSOFTHINGS(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectCLASSOFTHINGS(parent *Internals, user *User, n int, name string, b []byte) (*CLASSOFTHINGS, error) {
	fields := FieldsCLASSOFTHINGS{}
	object := user.NewCLASSOFTHINGS(parent, fields)
	...
	img, err := object.ValidateImageCLASSOFTHINGS(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeClassofthingsFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeClassofthingsFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}
...
func readZipFileCLASSOFTHINGS(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeClassofthingsFile(bucketName, objectName string, content []byte) error {
