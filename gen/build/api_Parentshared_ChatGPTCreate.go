...
func (app *App) parentChatGPTCreate(user *User, parent *PARENT, prompt string) error {
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewPARENT(&parent.Meta, FieldsPARENT{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentPARENT(&parent.Meta, object); err != nil {
			return err
		}
