...
import { TeststreetObjectGET, TeststreetMemberPOST } from './_fetch'

import { TeststreetMember } from './teststreetMember';

export function TeststreetMembers(props) {
	...
	function deleteAdmin(id) {
		...
		TeststreetMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			TeststreetMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<TeststreetAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
