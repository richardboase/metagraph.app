...
import { TeststreetsVertexPOST, TeststreetsChatGPTPOST, TeststreetsChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		case "prompt":
			...
			if (model == "openapi") {
				prom = TeststreetsVertexPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			} else {
				prom = TeststreetsChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			}
			prom.then((res) => {
				console.log(res)
		...
		case "create":
			...
			if (model == "openapi") {
				prom = TeststreetsVertexPOST(userdata, props.subject.Meta.ID, "create", payload)
			} else {
				prom = TeststreetsChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			}
			prom.then((res) => {
				console.log(res)
				props.updateList(true)
			...
			case "modify":
			TeststreetsChatGPTCollectionPOST(userdata, props.subject.Meta.ID, props.collection, payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
