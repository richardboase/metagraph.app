...
import { BookcharacterListRowImage } from './bookcharacterListRowImage';
import { BookcharacterDELETE, BookcharactersListGET, BookcharacterOrderPOST, BookcharacterJobPOST } from '../_fetch';


import { BookJobPOST } from '@/features/books/_fetch'
...
export function BookcharacterList(props) {
	...
	function sendToTopic(e) {
		...
		BookJobPOST(userdata, props.subject?.Meta.ID, job)
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "bookcharacters", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<BookcharacterListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
