...
import { BookcharacterList } from './shared/bookcharacterList';

import VisitTab from '../interfaces';

export function Bookcharacters(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newbookcharacter'} onClick={updateTabEvent} className="flex flex-col justify-center items-center m-2 cursor-pointer" style={buttonStyle}>
				...
				<AI subject={subject} updateList={updateList} collection="bookcharacters"/>
			</div>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <BookcharacterList title="Bookcharacters" subject={subject} native={true} />
			}
		</div>
	)
}