...
import { BookcharactersChatGPTPOST, BookcharactersChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		switch (select) {
		case "prompt":
			BookcharactersChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			.then((res) => {
				console.log(res)
			...
			break
		case "create":
			BookcharactersChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
