...
import { BookcharacterObjectGET, BookcharacterMemberPOST } from './_fetch'

import { BookcharacterMember } from './bookcharacterMember';

export function BookcharacterMembers(props) {
	...
	function deleteAdmin(id) {
		...
		BookcharacterMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			BookcharacterMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<BookcharacterAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
