...
import { JellynameObjectGET, JellynameMemberPOST } from './_fetch'

import { JellynameMember } from './jellynameMember';

export function JellynameMembers(props) {
	...
	function deleteAdmin(id) {
		...
		JellynameMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			JellynameMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<JellynameAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
