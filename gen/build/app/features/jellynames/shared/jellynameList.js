...
import { JellynameListRow } from './jellynameListRow';
import { JellynameListRowJob } from './jellynameListRowJob';
import { JellynameListRowImage } from './jellynameListRowImage';
import { JellynameDELETE, JellynamesListGET, JellynameOrderPOST, JellynameJobPOST } from '../_fetch';


import { ArthurJobPOST } from '@/features/arthurs/_fetch'


export function JellynameList(props) {
	...
	function updateList() {
		JellynamesListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectItem(id) {
		console.log("SELECT Jellyname", id)
		const next = list[parseInt(id)]
		const context = {
			"_": (next.Meta.Name ? next.fields.name : next.fields.name),
			"object": next,
		}
		setLocaldata(VisitTab(localdata, "jellyname", context))
	}

	function selectChild() {
		setLocaldata(VisitTab(localdata, "jellynames", context))
	}

	function moveUp(id) {
		const object = list[parseInt(id)]
		console.log("MOVE UP", object)
		JellynameOrderPOST(userdata, object.Meta.ID, "up")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function moveDown(id) {
		...
		JellynameOrderPOST(userdata, object.Meta.ID, "down")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function deleteItem(id) {
		...
		JellynameDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<JellynameListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
