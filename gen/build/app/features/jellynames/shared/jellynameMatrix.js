...
import { JellynameMatrixRow } from './jellynameMatrixRow';
import {
	JellynameDELETE,
	JellynamesListGET,
	JellynameOrderPOST,
...
export function JellynameMatrix(props) {
	...
	function updateList() {
		JellynamesListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newjellyname", localdata.tab.context))
	}
		...
		<table className='w-full'><tbody>
			...
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<JellynameMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
