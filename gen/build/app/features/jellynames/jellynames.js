...
import { JellynameList } from './shared/jellynameList';

import VisitTab from '../interfaces';

export function Jellynames(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newjellyname'} onClick={updateTabEvent} className="flex flex-col justify-center items-center m-2 cursor-pointer" style={buttonStyle}>
				...
				<AI subject={subject} updateList={updateList} collection="jellynames"/>
			</div>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <JellynameList title="Jellynames" subject={subject} native={true} />
			}
		</div>
	)
}