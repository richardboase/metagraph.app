...
import { ClassofthingsList } from './shared/classofthingsList';

import VisitTab from '../interfaces';

export function Classofthingss(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newclassofthings'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="classofthingss"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <ClassofthingsList title="Parents" subject={subject} native={true} />
			}
		</div>
	)
}