...
import { ClassofthingsMatrixRow } from './classofthingsMatrixRow';
import {
	ClassofthingsDELETE,
	ClassofthingssListGET,
	ClassofthingsOrderPOST,
} from '../_fetch';
import { ObjectPATCH } from '@/app/fetch'


export function ClassofthingsMatrix(props) {
	...
	function updateList() {
		ClassofthingssListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newclassofthings", localdata.tab.context))
	}
		...
		<table className='w-full'><tbody>
			...
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<ClassofthingsMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
