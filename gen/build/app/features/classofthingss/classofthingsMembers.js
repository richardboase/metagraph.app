...
import { ClassofthingsObjectGET, ClassofthingsMemberPOST } from './_fetch'

import { ClassofthingsMember } from './classofthingsMember';

export function ClassofthingsMembers(props) {
	...
	function deleteAdmin(id) {
		...
		ClassofthingsMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			ClassofthingsMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<ClassofthingsAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
