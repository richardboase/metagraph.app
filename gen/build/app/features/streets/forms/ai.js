...
import { StreetsVertexPOST, StreetsChatGPTPOST, StreetsChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		case "prompt":
			...
			if (model != "openai") {
				prom = StreetsVertexPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			} else {
				prom = StreetsChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			}
			prom.then((res) => {
				console.log(res)
		...
		case "create":
			...
			if (model != "openai") {
				prom = StreetsVertexPOST(userdata, props.subject.Meta.ID, "create", payload)
			} else {
				prom = StreetsChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			}
			prom.then((res) => {
				console.log(res)
				props.updateList(true)
			...
			case "modify":
			StreetsChatGPTCollectionPOST(userdata, props.subject.Meta.ID, props.collection, payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
