...
import { StreetMatrixRow } from './streetMatrixRow';
import {
	StreetDELETE,
	StreetsListGET,
	StreetOrderPOST,
...
export function StreetMatrix(props) {
	...
	function updateList() {
		StreetsListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newstreet", localdata.tab.context))
	}
	...
	function deleteItem(id) {
		...
		StreetDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	<>
	{
		...
		<table className='w-full'><tbody>
			<tr>
				...
				<td className='font-bold px-2' style={cellStyle}>name</td><td className='font-bold px-2' style={cellStyle}>zoning</td><td className='font-bold px-2' style={cellStyle}>length</td>
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<StreetMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
