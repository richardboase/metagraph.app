...
import { ParagraphChatGPTPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		switch (select) {
		case "prompt":
			ParagraphChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			.then((res) => {
				console.log(res)
		...
		case "create":
			...
			break
		case "modify":
			ParagraphChatGPTPOST(userdata, props.subject.Meta.ID, props.collection, "modify", payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
