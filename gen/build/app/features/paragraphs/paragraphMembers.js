...
import { ParagraphObjectGET, ParagraphMemberPOST } from './_fetch'

import { ParagraphMember } from './paragraphMember';

export function ParagraphMembers(props) {
	...
	function deleteAdmin(id) {
		...
		ParagraphMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			ParagraphMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<ParagraphAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
