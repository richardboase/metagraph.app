...
import { ThingListRow } from './thingListRow';
import { ThingListRowJob } from './thingListRowJob';
import { ThingListRowImage } from './thingListRowImage';
import { ThingDELETE, ThingsListGET, ThingOrderPOST, ThingJobPOST } from '../_fetch';


import { RoomJobPOST } from '@/features/rooms/_fetch'


export function ThingList(props) {
	...
	function updateList() {
		ThingsListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectItem(id) {
		console.log("SELECT Thing", id)
		const next = list[parseInt(id)]
		const context = {
			"_": (next.Meta.Name ? next.fields.name : next.fields.name),
			"object": next,
		}
		setLocaldata(VisitTab(localdata, "thing", context))
	}

	function selectChild() {
		setLocaldata(VisitTab(localdata, "things", context))
	}

	function moveUp(id) {
		const object = list[parseInt(id)]
		console.log("MOVE UP", object)
		ThingOrderPOST(userdata, object.Meta.ID, "up")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function moveDown(id) {
		...
		ThingOrderPOST(userdata, object.Meta.ID, "down")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function deleteItem(id) {
		...
		ThingDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<ThingListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
