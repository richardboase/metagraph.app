...
import { CharacterListRowJob } from './characterListRowJob';
import { CharacterListRowImage } from './characterListRowImage';
import { CharacterDELETE, CharactersListGET, CharacterOrderPOST, CharacterJobPOST } from '../_fetch';


import { LobbyJobPOST } from '@/features/lobbys/_fetch'
...
export function CharacterList(props) {
	...
	function updateList() {
		CharactersListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function sendToTopic(e) {
		...
		LobbyJobPOST(userdata, props.subject?.Meta.ID, job)
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "characters", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<CharacterListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
