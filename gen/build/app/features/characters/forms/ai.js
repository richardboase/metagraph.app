...
import { CharacterChatGPTModifyPOST } from '../_fetch'
import { CharacterChatGPTInitPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		if (select == "create") {
			CharacterChatGPTInitPOST(userdata, props.subject.Meta.ID, payload)
			.then((res) => {
				console.log(res)
			...
			CharacterChatGPTModifyPOST(userdata, props.subject.Meta.ID, props.collection, payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
