...
import { HealthcheckupAssetsRow } from './healthcheckupAssetsRow';
import { HealthcheckupDELETE, HealthcheckupOrderPOST } from '../_fetch';

export function HealthcheckupAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<HealthcheckupAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
