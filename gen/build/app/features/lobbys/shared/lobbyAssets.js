...
import { LobbyDELETE, LobbyOrderPOST } from '../_fetch';
...
export function LobbyAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<LobbyAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
