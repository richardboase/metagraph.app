...
import { LobbyListRowImage } from './lobbyListRowImage';
import { LobbyDELETE, LobbysListGET, LobbyOrderPOST, LobbyJobPOST } from '../_fetch';
...
export function LobbyList(props) {
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "lobbys", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<LobbyListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
