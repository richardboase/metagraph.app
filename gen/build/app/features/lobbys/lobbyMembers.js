...
import { LobbyObjectGET, LobbyMemberPOST } from './_fetch'

import { LobbyMember } from './lobbyMember';

export function LobbyMembers(props) {
	...
	function deleteAdmin(id) {
		...
		LobbyMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			LobbyMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<LobbyAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
