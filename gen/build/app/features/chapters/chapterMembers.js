...
import { ChapterObjectGET, ChapterMemberPOST } from './_fetch'

import { ChapterMember } from './chapterMember';

export function ChapterMembers(props) {
	...
	function deleteAdmin(id) {
		...
		ChapterMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			ChapterMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<ChapterAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
