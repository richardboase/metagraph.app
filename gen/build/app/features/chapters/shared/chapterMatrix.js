...
import { ChapterMatrixRow } from './chapterMatrixRow';
import {
	ChapterDELETE,
	ChaptersListGET,
	ChapterMoveUpPOST,
...
export function ChapterMatrix(props) {
	...
	function updateList() {
		ChaptersListGET(userdata, props.subject?.Meta.ID, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newchapter", localdata.tab.context))
	}
		...
		<table className='w-full'><tbody>
			...
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<ChapterMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
