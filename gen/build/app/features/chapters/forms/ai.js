...
import { ChapterChatGPTModifyPOST } from '../_fetch'
import { ChapterChatGPTInitPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		if (select == "create") {
			ChapterChatGPTInitPOST(userdata, props.subject.Meta.ID, payload)
			.then((res) => {
				console.log(res)
			...
			ChapterChatGPTModifyPOST(userdata, props.subject.Meta.ID, props.collection, payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
