...
import { ChaptersChatGPTPOST, ChaptersChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		switch (select) {
		case "prompt":
			ChaptersChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			.then((res) => {
				console.log(res)
			...
			break
		case "create":
			ChaptersChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
