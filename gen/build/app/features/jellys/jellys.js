...
import { JellyList } from './shared/jellyList';

import VisitTab from '../interfaces';

export function Jellys(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'inituploadjelly'} onClick={updateTabEvent} className="flex flex-col justify-center items-center m-2 cursor-pointer" style={buttonStyle}>
				...
				<AI subject={subject} updateList={updateList} collection="jellys"/>
			</div>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <JellyList title="Jellies" subject={subject} native={true} />
			}
		</div>
	)
}