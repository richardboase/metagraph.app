...
import { JellyMatrixRow } from './jellyMatrixRow';
import {
	JellyDELETE,
	JellysListGET,
	JellyOrderPOST,
...
export function JellyMatrix(props) {
	...
	function updateList() {
		JellysListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newjelly", localdata.tab.context))
	}
		...
		<table className='w-full'><tbody>
			...
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<JellyMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
