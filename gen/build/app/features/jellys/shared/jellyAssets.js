...
import { JellyAssetsRow } from './jellyAssetsRow';
import { JellyDELETE, JellyOrderPOST } from '../_fetch';

export function JellyAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<JellyAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
