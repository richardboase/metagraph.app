...
import { ArthurList } from './shared/arthurList';

import VisitTab from '../interfaces';

export function Arthurs(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newarthur'} onClick={updateTabEvent} className="flex flex-col justify-center items-center m-2 cursor-pointer" style={buttonStyle}>
				...
				<AI subject={subject} updateList={updateList} collection="arthurs"/>
			</div>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <ArthurList title="Arthurs" subject={subject} native={true} />
			}
		</div>
	)
}