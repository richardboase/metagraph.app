...
import { ArthurAssetsRow } from './arthurAssetsRow';
import { ArthurDELETE, ArthurOrderPOST } from '../_fetch';

export function ArthurAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<ArthurAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
