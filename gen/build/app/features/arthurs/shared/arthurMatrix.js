...
import { ArthurMatrixRow } from './arthurMatrixRow';
import {
	ArthurDELETE,
	ArthursListGET,
	ArthurOrderPOST,
...
export function ArthurMatrix(props) {
	...
	function updateList() {
		ArthursListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newarthur", localdata.tab.context))
	}
		...
		<table className='w-full'><tbody>
			...
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<ArthurMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
