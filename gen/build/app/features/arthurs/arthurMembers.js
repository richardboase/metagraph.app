...
import { ArthurObjectGET, ArthurMemberPOST } from './_fetch'

import { ArthurMember } from './arthurMember';

export function ArthurMembers(props) {
	...
	function deleteAdmin(id) {
		...
		ArthurMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			ArthurMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<ArthurAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
