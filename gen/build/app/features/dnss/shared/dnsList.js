...
import { DnsListRow } from './dnsListRow';
import { DnsListRowJob } from './dnsListRowJob';
import { DnsListRowImage } from './dnsListRowImage';
import { DnsDELETE, DnssListGET, DnsOrderPOST, DnsJobPOST } from '../_fetch';



export function DnsList(props) {
	...
	function updateList() {
		DnssListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectItem(id) {
		console.log("SELECT Dns", id)
		const next = list[parseInt(id)]
		const context = {
			"_": (next.Meta.Name ? next.fields.name : next.fields.name),
			"object": next,
		}
		setLocaldata(VisitTab(localdata, "dns", context))
	}

	function selectChild() {
		setLocaldata(VisitTab(localdata, "dnss", context))
	}

	function moveUp(id) {
		const object = list[parseInt(id)]
		console.log("MOVE UP", object)
		DnsOrderPOST(userdata, object.Meta.ID, "up")
	...
	function moveDown(id) {
		...
		DnsOrderPOST(userdata, object.Meta.ID, "down")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function deleteItem(id) {
		...
		DnsDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<DnsListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
