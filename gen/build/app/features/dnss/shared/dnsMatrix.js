...
import { DnsMatrixRow } from './dnsMatrixRow';
import {
	DnsDELETE,
	DnssListGET,
	DnsOrderPOST,
...
export function DnsMatrix(props) {
	...
	function updateList() {
		DnssListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newdns", localdata.tab.context))
	}
		...
		<table className='w-full'><tbody>
			...
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<DnsMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
