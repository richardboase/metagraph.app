...
import { DnsList } from './shared/dnsList';

import VisitTab from '../interfaces';

export function Dnss(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newdns'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="dnss"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <DnsList title="Dnss" subject={subject} native={true} />
			}
		</div>
	)
}