...
import { DnsObjectGET, DnsAdminPOST } from './_fetch'

import { DnsAdmin } from './dnsAdmin';

export function DnsAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		DnsAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			DnsAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
