...
import { AdopterObjectGET, AdopterMemberPOST } from './_fetch'

import { AdopterMember } from './adopterMember';

export function AdopterMembers(props) {
	...
	function deleteAdmin(id) {
		...
		AdopterMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			AdopterMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<AdopterAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
