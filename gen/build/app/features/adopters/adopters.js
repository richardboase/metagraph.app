...
import { AdopterList } from './shared/adopterList';

import VisitTab from '../interfaces';

export function Adopters(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newadopter'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="adopters"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <AdopterList title="Adopters" subject={subject} native={true} />
			}
		</div>
	)
}