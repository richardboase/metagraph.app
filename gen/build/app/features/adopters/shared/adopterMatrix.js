...
import { AdopterMatrixRow } from './adopterMatrixRow';
import {
	AdopterDELETE,
	AdoptersListGET,
	AdopterOrderPOST,
...
export function AdopterMatrix(props) {
	...
	function updateList() {
		AdoptersListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newadopter", localdata.tab.context))
	}
	...
	<>
	{
		...
		<table className='w-full'><tbody>
			<tr>
				...
				<td className='font-bold px-2' style={cellStyle}>fullname</td><td className='font-bold px-2' style={cellStyle}>contactinfo</td><td className='font-bold px-2' style={cellStyle}>address</td>
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<AdopterMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
