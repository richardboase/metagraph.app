...
import { AdopterObjectGET, AdopterAdminPOST } from './_fetch'

import { AdopterAdmin } from './adopterAdmin';

export function AdopterAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		AdopterAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			AdopterAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
