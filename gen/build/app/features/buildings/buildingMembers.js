...
import { BuildingObjectGET, BuildingMemberPOST } from './_fetch'

import { BuildingMember } from './buildingMember';

export function BuildingMembers(props) {
	...
	function deleteAdmin(id) {
		...
		BuildingMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			BuildingMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<BuildingAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
