...
import { QuarterObjectGET, QuarterAdminPOST } from './_fetch'

import { QuarterAdmin } from './quarterAdmin';

export function QuarterAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		QuarterAdminPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			QuarterAdminPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<QuarterAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
