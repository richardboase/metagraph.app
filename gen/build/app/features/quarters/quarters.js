...
import { QuarterList } from './shared/quarterList';

import VisitTab from '../interfaces';

export function Quarters(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newquarter'} onClick={updateTabEvent} className="flex flex-col justify-center items-center m-2 cursor-pointer" style={buttonStyle}>
			...
			{
				promptToggle && <QuarterList title="Quarters" subject={subject} native={true} />
			}
		</div>
	)
}