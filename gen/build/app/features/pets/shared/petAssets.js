...
import { PetAssetsRow } from './petAssetsRow';
import { PetDELETE, PetOrderPOST } from '../_fetch';

export function PetAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<PetAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
