...
import { PetObjectGET, PetAdminPOST } from './_fetch'

import { PetAdmin } from './petAdmin';

export function PetAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		PetAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			PetAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
