		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newbook'} onClick={updateTabEvent} className="flex flex-col justify-center items-center m-2 cursor-pointer" style={buttonStyle}>
				...
				<AI subject={subject} updateList={updateList} collection="books"/>
			</div>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <BookList title="Books" subject={subject} native={true} />
			}
		</div>
	)
}