...
import { BookListRowJob } from './bookListRowJob';
import { BookListRowImage } from './bookListRowImage';
import { BookDELETE, BooksListGET, BookOrderPOST, BookJobPOST } from '../_fetch';
...
export function BookList(props) {
	...
	function updateList() {
		BooksListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "books", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<BookListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
