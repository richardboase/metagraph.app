...
import { DonationListRowJob } from './donationListRowJob';
import { DonationListRowImage } from './donationListRowImage';
import { DonationDELETE, DonationsListGET, DonationOrderPOST, DonationJobPOST } from '../_fetch';
...
export function DonationList(props) {
	...
	function updateList() {
		DonationsListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectItem(id) {
		console.log("SELECT Donation", id)
		const next = list[parseInt(id)]
		const context = {
			"_": (next.Meta.Name ? next.fields.name : next.fields.name),
			"object": next,
		}
		setLocaldata(VisitTab(localdata, "donation", context))
	}

	function selectChild() {
		setLocaldata(VisitTab(localdata, "donations", context))
	}

	function moveUp(id) {
		const object = list[parseInt(id)]
		console.log("MOVE UP", object)
		DonationOrderPOST(userdata, object.Meta.ID, "up")
	...
	function moveDown(id) {
		...
		DonationOrderPOST(userdata, object.Meta.ID, "down")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function deleteItem(id) {
		...
		DonationDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<DonationListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
