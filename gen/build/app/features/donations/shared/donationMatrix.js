...
import { DonationMatrixRow } from './donationMatrixRow';
import {
	DonationDELETE,
	DonationsListGET,
	DonationOrderPOST,
...
export function DonationMatrix(props) {
	...
	function updateList() {
		DonationsListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newdonation", localdata.tab.context))
	}
	...
	<>
	{
		...
		<table className='w-full'><tbody>
			<tr>
				...
				<td className='font-bold px-2' style={cellStyle}>amount</td><td className='font-bold px-2' style={cellStyle}>donorname</td>
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<DonationMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
