...
import { DonationObjectGET, DonationJobPOST } from './_fetch'
...
export function Donation(props) {  
	...
	function editData() {
		setLocaldata(VisitTab(localdata, "editdonation", localdata.tab.context))
	}
						...
						<tbody>
							...
							<tr className='flex flex-row'>
								...
								<td className='flex flex-col justify-start'>
									<div className='w-full flex flex-row justify-end'>
										{
											(typeof subject.fields["amount"] === 'object') && <div className='flex flex-col m-4'>
												{
													Object.keys(subject.fields["amount"]).forEach(function(k, i) {
														const v = subject.fields["amount"][k]
														return (
										...
										{
											Array.isArray(subject.fields["amount"]) && subject.fields["amount"].map(function(item, i) {
												return (
										...
										{
											!Array.isArray(subject.fields["amount"]) && !(typeof subject.fields["amount"] === 'object') && <>
												
												
												{ subject.fields["amount"] }
							...
							<tr className='flex flex-row'>
								...
								<td className='flex flex-col justify-start'>
									<div className='w-full flex flex-row justify-end'>
										{
											(typeof subject.fields["donorname"] === 'object') && <div className='flex flex-col m-4'>
												{
													Object.keys(subject.fields["donorname"]).forEach(function(k, i) {
														const v = subject.fields["donorname"][k]
														return (
															<div key={i} className='flex flex-row text-xs m-2'>
																<div className=''>{k}</div>
																<div className='px-2'>:</div>
										...
										{
											Array.isArray(subject.fields["donorname"]) && subject.fields["donorname"].map(function(item, i) {
												return (
													<div key={i} className='text-xs'>{item}</div>
												)
											})
										...
										{
											!Array.isArray(subject.fields["donorname"]) && !(typeof subject.fields["donorname"] === 'object') && <>
												
												
												{ subject.fields["donorname"] }
