...
import { DonationObjectGET, DonationAdminPOST } from './_fetch'

import { DonationAdmin } from './donationAdmin';

export function DonationAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		DonationAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			DonationAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
