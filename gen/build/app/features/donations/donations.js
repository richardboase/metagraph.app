...
import { DonationList } from './shared/donationList';
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newdonation'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="donations"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <DonationList title="Donations" subject={subject} native={true} />
			}
		</div>
	)
}