...
import { ParentListRowJob } from './parentListRowJob';
import { ParentListRowImage } from './parentListRowImage';
import { ParentDELETE, ParentsListGET, ParentOrderPOST, ParentJobPOST } from '../_fetch';



export function ParentList(props) {
	...
	function updateList() {
		ParentsListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "parents", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<ParentListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
