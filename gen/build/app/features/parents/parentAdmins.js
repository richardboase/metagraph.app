...
import { ParentObjectGET, ParentAdminPOST } from './_fetch'

import { ParentAdmin } from './parentAdmin';

export function ParentAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		ParentAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			ParentAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
