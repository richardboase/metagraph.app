...
import { CreatorMatrixRow } from './creatorMatrixRow';
import {
	CreatorDELETE,
	CreatorsListGET,
	CreatorOrderPOST,
...
export function CreatorMatrix(props) {
	...
	function updateList() {
		CreatorsListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function newobject() {
		setLocaldata(VisitTab(localdata, "newcreator", localdata.tab.context))
	}
	...
	function selectItem(id) {
		console.log("SELECT Creator", id)
		const next = list[parseInt(id)]
		const context = {
			"_": next.fields.name,
			"object": next,
		}
		setLocaldata(VisitTab(localdata, "creator", context))
	}

	function moveUp(id) {
		const object = list[parseInt(id)]
		console.log("MOVE UP", object)
		CreatorOrderPOST(userdata, object.Meta.ID, "up")
	...
	function moveDown(id) {
		...
		CreatorOrderPOST(userdata, object.Meta.ID, "down")
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	function deleteItem(id) {
		...
		CreatorDELETE(userdata, object.Meta.ID)
		.then((res) => console.log(res))
		.then(function () {
			updateList()
		})
	...
	<>
	{
		...
		<table className='w-full'><tbody>
			<tr>
				...
				<td className='font-bold px-2' style={cellStyle}>name</td><td className='font-bold px-2' style={cellStyle}>company</td><td className='font-bold px-2' style={cellStyle}>band</td><td className='font-bold px-2' style={cellStyle}>artistname</td>
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<CreatorMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
