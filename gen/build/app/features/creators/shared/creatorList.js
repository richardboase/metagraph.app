...
import { CreatorListRowJob } from './creatorListRowJob';
import { CreatorListRowImage } from './creatorListRowImage';
import { CreatorDELETE, CreatorsListGET, CreatorOrderPOST, CreatorJobPOST } from '../_fetch';



export function CreatorList(props) {
	...
	function updateList() {
		CreatorsListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "creators", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<CreatorListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
