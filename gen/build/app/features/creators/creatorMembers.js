...
import { CreatorObjectGET, CreatorMemberPOST } from './_fetch'

import { CreatorMember } from './creatorMember';

export function CreatorMembers(props) {
	...
	function deleteAdmin(id) {
		...
		CreatorMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			CreatorMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<CreatorAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
