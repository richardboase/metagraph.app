...
import { GamesVertexPOST, GamesChatGPTPOST, GamesChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		case "prompt":
			...
			if (model != "openai") {
				prom = GamesVertexPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			} else {
				prom = GamesChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			}
			prom.then((res) => {
				console.log(res)
		...
		case "create":
			...
			if (model != "openai") {
				prom = GamesVertexPOST(userdata, props.subject.Meta.ID, "create", payload)
			} else {
				prom = GamesChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			}
			prom.then((res) => {
				console.log(res)
				props.updateList(true)
