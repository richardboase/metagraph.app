...
import { GamingcarddetailsAssetsRow } from './gamingcarddetailsAssetsRow';
import { GamingcarddetailsDELETE, GamingcarddetailsOrderPOST } from '../_fetch';

export function GamingcarddetailsAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<GamingcarddetailsAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
