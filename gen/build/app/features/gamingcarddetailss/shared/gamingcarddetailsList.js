...
import { GamingcarddetailsListRow } from './gamingcarddetailsListRow';
import { GamingcarddetailsListRowJob } from './gamingcarddetailsListRowJob';
import { GamingcarddetailsListRowImage } from './gamingcarddetailsListRowImage';
import { GamingcarddetailsDELETE, GamingcarddetailssListGET, GamingcarddetailsOrderPOST, GamingcarddetailsJobPOST } from '../_fetch';


import { TokenJobPOST } from '@/features/tokens/_fetch'


export function GamingcarddetailsList(props) {
	...
	function updateList() {
		GamingcarddetailssListGET(userdata, props.subject?.Meta.ID, listMode, props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	function selectChild() {
		setLocaldata(VisitTab(localdata, "gamingcarddetailss", context))
	}
			...
			list && list.map(function (item, i) {
				...
				return (
					<div className=' py-2 px-4' key={i}>
							...
							<GamingcarddetailsListRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
