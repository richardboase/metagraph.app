...
import { GamingcarddetailsObjectGET, GamingcarddetailsMemberPOST } from './_fetch'

import { GamingcarddetailsMember } from './gamingcarddetailsMember';

export function GamingcarddetailsMembers(props) {
	...
	function deleteAdmin(id) {
		...
		GamingcarddetailsMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			GamingcarddetailsMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<GamingcarddetailsAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
