...
import { GamingcarddetailsList } from './shared/gamingcarddetailsList';

import VisitTab from '../interfaces';

export function Gamingcarddetailss(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newgamingcarddetails'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="gamingcarddetailss"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <GamingcarddetailsList title="Gamingcarddetailss" subject={subject} native={true} />
			}
		</div>
	)
}