...
import { GamingcarddetailssModelsPOST, GamingcarddetailssChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		case "prompt":
			...
			GamingcarddetailssModelsPOST(userdata, props.subject.Meta.ID, model, mode, payload)
			.then((res) => {
				console.log(res)
		...
		case "modify":
			GamingcarddetailssChatGPTCollectionPOST(userdata, props.subject.Meta.ID, props.collection, payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
