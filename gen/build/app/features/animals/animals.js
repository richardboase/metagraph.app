...
import { AnimalList } from './shared/animalList';
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newanimal'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="animals"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <AnimalList title="Animals" subject={subject} native={true} />
			}
		</div>
	)
}