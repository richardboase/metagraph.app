...
import { AnimalObjectGET, AnimalMemberPOST } from './_fetch'

import { AnimalMember } from './animalMember';

export function AnimalMembers(props) {
	...
	function deleteAdmin(id) {
		...
		AnimalMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			AnimalMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<AnimalAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
