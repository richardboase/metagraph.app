...
import { AnimalAssetsRow } from './animalAssetsRow';
import { AnimalDELETE, AnimalOrderPOST } from '../_fetch';

export function AnimalAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<AnimalAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
