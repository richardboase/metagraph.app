...
import { AnimalObjectGET, AnimalAdminPOST } from './_fetch'

import { AnimalAdmin } from './animalAdmin';

export function AnimalAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		AnimalAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			AnimalAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
