...
import { MusicdetailsList } from './shared/musicdetailsList';

import VisitTab from '../interfaces';

export function Musicdetailss(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newmusicdetails'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="musicdetailss"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <MusicdetailsList title="Musicdetailss" subject={subject} native={true} />
			}
		</div>
	)
}