...
import { MusicdetailsObjectGET, MusicdetailsMemberPOST } from './_fetch'

import { MusicdetailsMember } from './musicdetailsMember';

export function MusicdetailsMembers(props) {
	...
	function deleteAdmin(id) {
		...
		MusicdetailsMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			MusicdetailsMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<MusicdetailsAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
