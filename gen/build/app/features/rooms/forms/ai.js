...
import { RoomsVertexPOST, RoomsChatGPTPOST, RoomsChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		case "prompt":
			...
			if (model != "openai") {
				prom = RoomsVertexPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			} else {
				prom = RoomsChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			}
			prom.then((res) => {
				console.log(res)
		...
		case "create":
			...
			if (model != "openai") {
				prom = RoomsVertexPOST(userdata, props.subject.Meta.ID, "create", payload)
			} else {
				prom = RoomsChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			}
			prom.then((res) => {
				console.log(res)
				props.updateList(true)
		...
		case "modify":
			RoomsChatGPTCollectionPOST(userdata, props.subject.Meta.ID, props.collection, payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
