...
import { RoomMatrixRow } from './roomMatrixRow';
import {
	RoomDELETE,
	RoomsListGET,
	RoomOrderPOST,
...
export function RoomMatrix(props) {
	...
	function updateList() {
		RoomsListGET(userdata, props.subject?.Meta.ID, "admin", props.limit)
		.then((res) => res.json())
		.then((data) => {
			console.log(data)
			setList(data)
	...
	<>
	{
		...
		<table className='w-full'><tbody>
			<tr>
				...
				<td className='font-bold px-2' style={cellStyle}>name</td><td className='font-bold px-2' style={cellStyle}>descriptoion</td>
			</tr>
			{
				list && list.map(function (row, i) {
					return (
						<RoomMatrixRow id={i} key={i} row={row} save={saveUpdate}/>
					)
				})
			}
			<tr>
