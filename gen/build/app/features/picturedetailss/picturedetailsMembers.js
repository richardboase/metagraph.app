...
import { PicturedetailsObjectGET, PicturedetailsMemberPOST } from './_fetch'

import { PicturedetailsMember } from './picturedetailsMember';

export function PicturedetailsMembers(props) {
	...
	function deleteAdmin(id) {
		...
		PicturedetailsMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			PicturedetailsMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<PicturedetailsAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
