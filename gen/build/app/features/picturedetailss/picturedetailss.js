...
import { PicturedetailsList } from './shared/picturedetailsList';

import VisitTab from '../interfaces';

export function Picturedetailss(props) {
		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newpicturedetails'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="picturedetailss"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <PicturedetailsList title="Picturedetailss" subject={subject} native={true} />
			}
		</div>
	)
}