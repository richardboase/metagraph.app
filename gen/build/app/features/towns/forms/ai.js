...
import { TownsChatGPTPOST, TownsChatGPTCollectionPOST } from '../_fetch'
...
export function AI(props) {
	...
	function sendPrompt() {
		...
		switch (select) {
		case "prompt":
			TownsChatGPTPOST(userdata, props.subject.Meta.ID, "prompt", payload)
			.then((res) => {
				console.log(res)
			...
			break
		case "create":
			TownsChatGPTPOST(userdata, props.subject.Meta.ID, "create", payload)
			.then((res) => {
				console.log(res)
				props.updateList(true)
			}) 
