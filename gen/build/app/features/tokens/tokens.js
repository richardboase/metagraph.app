		...
		<div style={ {padding:"30px 60px 30px 60px"} } className='flex flex-col w-full'>
			<div className='flex flex-row justify-between w-full'>
				<div className='flex flex-row'>
					...
					<button id={'newtoken'} onClick={updateTabEvent} className="flex flex-col justify-center items-center cursor-pointer" style={buttonStyle}>
			...
			<AI subject={subject} updateList={updateList} collection="tokens"/>
			{
				!promptToggle && <Loading/>
			}
			{
				promptToggle && <TokenList title="Tokens" subject={subject} native={true} />
			}
		</div>
	)
}