...
import { TokenObjectGET, TokenAdminPOST } from './_fetch'

import { TokenAdmin } from './tokenAdmin';

export function TokenAdmins(props) {
	...
	function deleteAdmin(id) {
		...
		TokenAdminPOST(userdata, object.Meta.ID, "remove", adminID)
		.then(function () {
			updateObject()
		}).error(function (e) {
			console.error(e)
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			TokenAdminPOST(userdata, object.Meta.ID, "add", admin)
			.then(function () {
				updateObject()
			}).error(function (e) {
				console.error(e)
