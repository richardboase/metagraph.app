...
import { FurnishingObjectGET, FurnishingMemberPOST } from './_fetch'

import { FurnishingMember } from './furnishingMember';

export function FurnishingMembers(props) {
	...
	function deleteAdmin(id) {
		...
		FurnishingMemberPOST(userdata, project.Meta.ID, "remove", adminID)
		.then(updateProject)
	}
	...
	function addAdmins() {
		newAdmins.forEach(function (admin, i) {
			FurnishingMemberPOST(userdata, project.Meta.ID, "add", admin)
			.then(updateProject)
		})
	}
			...
			<div className='flex flex-col'>
				{
					project.Meta.Moderation.Admins.map(function (adminID, i) {
						return (
							<FurnishingAdmin key={adminID} id={i} admin={adminID} delete={deleteAdmin}/>
						)
					})
				}
			</div>
