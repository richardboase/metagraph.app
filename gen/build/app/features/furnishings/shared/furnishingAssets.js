...
import { FurnishingAssetsRow } from './furnishingAssetsRow';
import { FurnishingDELETE, FurnishingOrderPOST } from '../_fetch';

export function FurnishingAssets(props) {
	...
	}
	{
		list && list.map(function (item, i) {
			...
			return (
				<div key={i}>
					<FurnishingAssetsRow id={i} listLength={list.length} item={item} select={selectItem} moveUp={moveUp} moveDown={moveDown} delete={deleteItem}/>
					<Spacer/>
				</div>
			)
		})
