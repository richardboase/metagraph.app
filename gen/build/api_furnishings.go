...
// api-furnishings
func (app *App) EntrypointFURNISHINGS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "create":
				...
				if err := app.furnishingChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.furnishingChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "vertex":
			...
			case "create":
				...
				if err := app.furnishingVertexCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "init":
			...
			if err := app.CreateDocumentFURNISHING(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadFURNISHING(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of furnishings
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "furnishings"),
			}
		...
		case "list":
			...
			app.furnishingLists(w, user, parent, mode, limit)
