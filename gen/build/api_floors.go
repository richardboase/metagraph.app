...
func (app *App) EntrypointFLOORS(w http.ResponseWriter, r *http.Request) {
		...
		case "ai":
			...
			case "openai":
				...
				case "create":
					...
					if err := app.floorChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.floorChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.floorVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
	...
	case "GET":
		...
		case "list":
			...
			app.floorLists(w, user, parent, mode, limit)
