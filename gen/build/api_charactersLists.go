...
func (app *App) characterLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("characters")
	} else {
		collection = app.Firestore().Collection("characters")
	}
