...
// api-dnss
func (app *App) EntrypointDNSS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := FieldsDNS{}
			object := user.NewDNS(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentDNS(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of dnss
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "dnss"),
			}
		...
		case "list":
			...
			app.dnsLists(w, user, nil, mode, limit)
