...
func (app *App) UploadHEALTHCHECKUP(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectHEALTHCHECKUP(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileHEALTHCHECKUP(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectHEALTHCHECKUP(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentHEALTHCHECKUP(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectHEALTHCHECKUP(parent *Internals, user *User, n int, name string, b []byte) (*HEALTHCHECKUP, error) {
	...
	img, err := object.ValidateImageHEALTHCHECKUP(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeHealthcheckupFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeHealthcheckupFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}

	object.Meta.Media.Preview = "https://storage.googleapis.com/pet-rescue-center-uploads/" + uri

	return object, nil
}

func readZipFileHEALTHCHECKUP(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeHealthcheckupFile(bucketName, objectName string, content []byte) error {
