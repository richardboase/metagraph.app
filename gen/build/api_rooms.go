...
func (app *App) EntrypointROOMS(w http.ResponseWriter, r *http.Request) {
		...
		case "ai":
			...
			case "openai":
				...
				case "prompt":
					...
					reply, err := app.roomChatGPTPrompt(user, object, prompt)
					if err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "create":
					...
					if err := app.roomChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.roomChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.roomVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
	...
	case "GET":
		...
		case "list":
			...
			app.roomLists(w, user, parent, mode, limit)
