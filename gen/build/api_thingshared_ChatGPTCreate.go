...
func (app *App) thingChatGPTCreate(user *User, parent *THING, prompt string) error {
...
We want to create one or more of these data objects: a distinct ant transferrable object of any size, could be anything

...for this parent object: ` + parentString + `

{

	// the shortest description of the object  (THIS FIELD IS REQUIRED)
	name (string)

	// a full description of the object  (THIS FIELD IS REQUIRED)
	description (string)

	// the state of the object, for example: a tumbler could be "half full with water"  (THIS FIELD IS REQUIRED)
	state (string)
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewTHING(&parent.Meta, FieldsTHING{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentTHING(&parent.Meta, object); err != nil {
			return err
		}
