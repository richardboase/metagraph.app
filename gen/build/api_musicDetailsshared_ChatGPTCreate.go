...
func (app *App) musicdetailsChatGPTCreate(user *User, parent *MUSICDETAILS, prompt string) error {
...
We want to create one or more of these data objects: Additional details for music tokens

...for this parent object: ` + parentString + `

{

	// Name of the album  (THIS FIELD IS REQUIRED)
	albumname (string)

	// Name of the track  (THIS FIELD IS REQUIRED)
	trackname (string)

	// URL or reference to the album image 
	albumimage (string)

	// URL or reference to the track image 
	trackimage (string)
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewMUSICDETAILS(&parent.Meta, FieldsMUSICDETAILS{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentMUSICDETAILS(&parent.Meta, object); err != nil {
			return err
		}
