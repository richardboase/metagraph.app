...
// api-lobby
func (app *App) EntrypointLOBBY(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "prompt":
			...
			reply, err := app.lobbyChatGPTPrompt(object, prompt)
			if err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
...
func (app *App) getLobbyList(subject *LOBBY) []*LOBBY {
	list := []*LOBBY{}
