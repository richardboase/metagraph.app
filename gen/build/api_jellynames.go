...
// api-jellynames
func (app *App) EntrypointJELLYNAMES(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "prompt":
				...
				reply, err := app.jellynameChatGPTPrompt(user, object, prompt)
				if err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "create":
				...
				if err := app.jellynameChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.jellynameChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "vertex":
			...
			case "create":
				...
				if err := app.jellynameVertexCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "init":
			...
			if err := app.CreateDocumentJELLYNAME(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadJELLYNAME(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of jellynames
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "jellynames"),
			}
		...
		case "list":
			...
			app.jellynameLists(w, user, parent, mode, limit)
