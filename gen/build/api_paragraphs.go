...
func (app *App) EntrypointPARAGRAPHS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "prompt":
			...
			if err := app.paragraphChatGPTCreate(user, parent, prompt); err != nil {
		...
		case "init":
			...
			object := NewPARAGRAPH(parent, fields)
		...
		case "initupload":
			// reuse code
			app.UploadPARAGRAPH(w, r, parent)
			return
		*/
		...
		case "inituploads":
			// reuse code
			app.ArchiveUploadPARAGRAPH(w, r, parent)
			return
		*/
	...
	case "GET":
		...
		// return a list of paragraphs in a specific parent
		case "list", "altlist":
			...
			q := parent.Firestore(app.App).Collection("paragraphs").OrderBy("Meta.Modified", firestore.Desc)
			...
			for {
				...
				paragraph := &PARAGRAPH{}
				if err := doc.DataTo(paragraph); err != nil {
					log.Println(err)
					continue
				}
