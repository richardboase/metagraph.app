...
// api-creator
func (app *App) EntrypointCREATOR(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addCreatorAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeCreatorAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "prompt":
			...
			reply, err := app.creatorChatGPTPrompt(user, object, prompt)
			if err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				},
				{
					...
					Path: "fields.company",
					Value: object.Fields.Company,
				},{
					Path: "fields.band",
					Value: object.Fields.Band,
				},{
					Path: "fields.artistname",
					Value: object.Fields.Artistname,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getCreatorList(subject *CREATOR) []*CREATOR {
	list := []*CREATOR{}
