...
// api-classofthingss
func (app *App) EntrypointCLASSOFTHINGSS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			object := user.NewCLASSOFTHINGS(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentCLASSOFTHINGS(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of classofthingss
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "classofthingss"),
			}
		...
		case "list":
			...
			app.classofthingsLists(w, user, nil, mode, limit)
