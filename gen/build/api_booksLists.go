...
func (app *App) bookLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("books")
	} else {
		collection = app.Firestore().Collection("books")
	}
