...
func (app *App) chapterLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("chapters")
	} else {
		collection = app.Firestore().Collection("chapters")
	}
