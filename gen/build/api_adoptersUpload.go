...
func (app *App) UploadADOPTER(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectADOPTER(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileADOPTER(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectADOPTER(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentADOPTER(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectADOPTER(parent *Internals, user *User, n int, name string, b []byte) (*ADOPTER, error) {
	fields := FieldsADOPTER{}
	object := user.NewADOPTER(parent, fields)
	...
	img, err := object.ValidateImageADOPTER(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeAdopterFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeAdopterFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}
...
func readZipFileADOPTER(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeAdopterFile(bucketName, objectName string, content []byte) error {
