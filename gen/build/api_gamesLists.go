...
func (app *App) gameLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("games")
	} else {
		collection = app.Firestore().Collection("games")
	}
