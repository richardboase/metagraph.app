...
func (app *App) dnsChatGPTCreate(user *User, parent *DNS, prompt string) error {
...
We want to create one or more of these data objects: a domain trading platform
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewDNS(&parent.Meta, FieldsDNS{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentDNS(&parent.Meta, object); err != nil {
			return err
		}
