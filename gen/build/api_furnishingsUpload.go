...
func (app *App) UploadFURNISHING(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectFURNISHING(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileFURNISHING(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectFURNISHING(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentFURNISHING(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectFURNISHING(parent *Internals, user *User, n int, name string, b []byte) (*FURNISHING, error) {
	fields := FieldsFURNISHING{}
	object := user.NewFURNISHING(parent, fields)
	...
	img, err := object.ValidateImageFURNISHING(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeFurnishingFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeFurnishingFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}
...
func readZipFileFURNISHING(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeFurnishingFile(bucketName, objectName string, content []byte) error {
