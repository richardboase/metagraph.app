...
func (app *App) healthcheckupLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("healthcheckups")
	} else {
		collection = app.Firestore().Collection("healthcheckups")
	}
