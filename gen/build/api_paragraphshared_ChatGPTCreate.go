...
func (app *App) paragraphChatGPTCreate(user *User, parent *PARAGRAPH, prompt string) error {
...
We want to create one or more of these data objects: a paragraph in a chapter
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewPARAGRAPH(&parent.Meta, FieldsPARAGRAPH{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentPARAGRAPH(&parent.Meta, object); err != nil {
			return err
		}
