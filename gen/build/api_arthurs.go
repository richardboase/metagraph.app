...
// api-arthurs
func (app *App) EntrypointARTHURS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := FieldsARTHUR{}
			object := user.NewARTHUR(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentARTHUR(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of arthurs
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "arthurs"),
			}
		...
		case "list":
			...
			app.arthurLists(w, user, nil, mode, limit)
