...
func (app *App) UploadGAMINGCARDDETAILS(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	newFileObjects := []*GAMINGCARDDETAILS{}
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectGAMINGCARDDETAILS(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFileGAMINGCARDDETAILS(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectGAMINGCARDDETAILS(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentGAMINGCARDDETAILS(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectGAMINGCARDDETAILS(parent *Internals, user *User, n int, name string, b []byte) (*GAMINGCARDDETAILS, error) {
	fields := FieldsGAMINGCARDDETAILS{}
	object := user.NewGAMINGCARDDETAILS(parent, fields)
	...
	img, err := object.ValidateImageGAMINGCARDDETAILS(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writeGamingcarddetailsFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writeGamingcarddetailsFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}
...
func readZipFileGAMINGCARDDETAILS(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writeGamingcarddetailsFile(bucketName, objectName string, content []byte) error {
