...
// api-parents
func (app *App) EntrypointPARENTS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := FieldsPARENT{}
			object := user.NewPARENT(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentPARENT(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of parents
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "parents"),
			}
		...
		case "list":
			...
			app.parentLists(w, user, nil, mode, limit)
