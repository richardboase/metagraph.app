...
// api-chapters
func (app *App) EntrypointCHAPTERS(w http.ResponseWriter, r *http.Request) {
		...
		case "ai":
			...
			case "openai":
				...
				case "prompt":
					...
					reply, err := app.chapterChatGPTPrompt(user, object, prompt)
					if err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "create":
					...
					if err := app.chapterChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.chapterChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.chapterVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
	...
	case "GET":
		...
		// return the total amount of chapters
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "chapters"),
			}
		...
		case "list":
			...
			app.chapterLists(w, user, parent, mode, limit)
