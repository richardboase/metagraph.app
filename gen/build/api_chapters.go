...
func (app *App) EntrypointCHAPTERS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "prompt":
			...
			if err := app.chapterChatGPTCreate(user, parent, prompt); err != nil {
		...
		case "init":
			...
			object := NewCHAPTER(parent, fields)
		...
		*/
		...
		case "inituploads":
			// reuse code
			app.ArchiveUploadCHAPTER(w, r, parent)
			return
		*/
	...
	case "GET":
		...
		// return a list of chapters in a specific parent
		case "list", "altlist":
			...
			q := parent.Firestore(app.App).Collection("chapters").OrderBy("Meta.Context.Order", firestore.Asc)
