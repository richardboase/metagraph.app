...
// api-donation
func (app *App) EntrypointDONATION(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addDonationAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeDonationAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "prompt":
			...
			reply, err := app.donationChatGPTPrompt(user, object, prompt)
			if err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				{
					Path: "fields.amount",
					Value: object.Fields.Amount,
				},{
					Path: "fields.donorname",
					Value: object.Fields.Donorname,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getDonationList(subject *DONATION) []*DONATION {
	list := []*DONATION{}
