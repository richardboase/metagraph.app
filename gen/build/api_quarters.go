...
// api-quarters
func (app *App) EntrypointQUARTERS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "prompt":
				...
				reply, err := app.quarterChatGPTPrompt(user, object, prompt)
				if err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "create":
				...
				if err := app.quarterChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.quarterChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "vertex":
			...
			case "create":
				...
				if err := app.quarterVertexCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "upload":
			...
			app.UploadQUARTER(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of quarters
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "quarters"),
			}
		...
		case "list":
			...
			app.quarterLists(w, user, parent, mode, limit)
