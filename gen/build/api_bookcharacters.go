...
func (app *App) EntrypointBOOKCHARACTERS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "prompt":
			...
			if err := app.bookcharacterChatGPTCreate(user, parent, prompt); err != nil {
		...
		case "init":
			...
			object := NewBOOKCHARACTER(parent, fields)
		...
		case "initupload":
			// reuse code
			app.UploadBOOKCHARACTER(w, r, parent)
			return
		*/
		...
		case "inituploads":
			// reuse code
			app.ArchiveUploadBOOKCHARACTER(w, r, parent)
			return
		*/
	...
	case "GET":
		...
		// return a list of bookcharacters in a specific parent
		case "list", "altlist":
			...
			q := parent.Firestore(app.App).Collection("bookcharacters").OrderBy("Meta.Modified", firestore.Desc)
			...
			for {
				...
				if err := doc.DataTo(bookcharacter); err != nil {
					log.Println(err)
					continue
				}
				list = append(list, bookcharacter)
			}
