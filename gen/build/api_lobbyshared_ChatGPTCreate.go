...
func (app *App) lobbyChatGPTCreate(user *User, parent *LOBBY, prompt string) error {
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewLOBBY(&parent.Meta, FieldsLOBBY{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentLOBBY(&parent.Meta, object); err != nil {
			return err
		}
