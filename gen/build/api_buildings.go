...
func (app *App) EntrypointBUILDINGS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "prompt":
				...
				reply, err := app.buildingChatGPTPrompt(user, object, prompt)
				if err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "create":
				...
				if err := app.buildingChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.buildingChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "vertex":
			...
			case "create":
				...
				if err := app.buildingVertexCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "upload":
			...
			app.UploadBUILDING(w, r, parent, user)
			return
	...
	case "GET":
		...
		case "list":
			...
			app.buildingLists(w, user, parent, mode, limit)
