...
// api-picturedetailss
func (app *App) EntrypointPICTUREDETAILSS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "ai":
			...
			case "openai":
				...
				case "prompt":
					...
					reply, err := app.picturedetailsChatGPTPrompt(user, object, prompt)
					if err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "create":
					...
					if err := app.picturedetailsChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.picturedetailsChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.picturedetailsVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
		...
		case "init":
			...
			if err := app.CreateDocumentPICTUREDETAILS(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadPICTUREDETAILS(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of picturedetailss
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "picturedetailss"),
			}
		...
		case "list":
			...
			app.picturedetailsLists(w, user, parent, mode, limit)
