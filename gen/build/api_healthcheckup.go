...
// api-healthcheckup
func (app *App) EntrypointHEALTHCHECKUP(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addHealthcheckupAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeHealthcheckupAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
...
func (app *App) getHealthcheckupList(subject *HEALTHCHECKUP) []*HEALTHCHECKUP {
	list := []*HEALTHCHECKUP{}
