...
func (app *App) UploadPICTUREDETAILS(w http.ResponseWriter, r *http.Request, parent *Internals, user *User) {
	...
	for n, fileHeader := range files{
		...
		if !strings.Contains(strings.ToLower(fileHeader.Filename), "zip") {
			...
			obj, err := app.newUploadObjectPICTUREDETAILS(parent, user, 0, fileHeader.Filename, buf.Bytes())
			if err != nil {
				log.Println(err)
				return
			}
			...
			for n, zipFile := range zipReader.File {
				...
				extractedContent, err := readZipFilePICTUREDETAILS(zipFile)
				if err != nil {
					log.Println(err)
					continue
				}
		
				obj, err := app.newUploadObjectPICTUREDETAILS(parent, user, n, zipFile.Name, extractedContent)
				if err != nil {
					log.Println(err)
					continue
				}
	...
	for _, obj := range newFileObjects {
		if err := app.CreateDocumentPICTUREDETAILS(parent, obj); err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return		
		}
	}
...
func (app *App) newUploadObjectPICTUREDETAILS(parent *Internals, user *User, n int, name string, b []byte) (*PICTUREDETAILS, error) {
	fields := FieldsPICTUREDETAILS{}
	object := user.NewPICTUREDETAILS(parent, fields)
	...
	img, err := object.ValidateImagePICTUREDETAILS(b)
	if err != nil {
		fmt.Errorf("skipping file that cannot be decoded: %s", name)
		return nil, err
	}

	if err := app.writePicturedetailsFile(CONST_BUCKET_UPLOADS, uri, b); err != nil {
		return nil, err
	}
	...
	if err := app.writePicturedetailsFile(CONST_BUCKET_UPLOADS, uri, buf.Bytes()); err != nil {
		return nil, err
	}
...
func readZipFilePICTUREDETAILS(zipFile *zip.File) ([]byte, error) {
...
func (app *App) writePicturedetailsFile(bucketName, objectName string, content []byte) error {
