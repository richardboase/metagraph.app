...
func (app *App) dnsLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("dnss")
	} else {
		collection = app.Firestore().Collection("dnss")
	}
