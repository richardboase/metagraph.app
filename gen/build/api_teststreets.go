...
func (app *App) EntrypointTESTSTREETS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "prompt":
				...
				reply, err := app.teststreetChatGPTPrompt(user, object, prompt)
				if err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "create":
				...
				if err := app.teststreetChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.teststreetChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "vertex":
			...
			case "create":
				...
				if err := app.teststreetVertexCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "upload":
			...
			app.UploadTESTSTREET(w, r, parent, user)
			return
	...
	case "GET":
		...
		case "list":
			...
			app.teststreetLists(w, user, parent, mode, limit)
