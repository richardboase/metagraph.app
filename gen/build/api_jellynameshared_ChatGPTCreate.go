...
func (app *App) jellynameChatGPTCreate(user *User, parent *JELLYNAME, prompt string) error {
...
We want to create one or more of these data objects: arthurs 

...for this parent object: ` + parentString + `

{

	// the elemental name of the jelly  (THIS FIELD IS REQUIRED)
	name (string)
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewJELLYNAME(&parent.Meta, FieldsJELLYNAME{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentJELLYNAME(&parent.Meta, object); err != nil {
			return err
		}
