...
// api-gamingcarddetails
func (app *App) EntrypointGAMINGCARDDETAILS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addGamingcarddetailsAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeGamingcarddetailsAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				{
					Path: "fields.gametitle",
					Value: object.Fields.Gametitle,
				},{
					Path: "fields.cardtype",
					Value: object.Fields.Cardtype,
				},{
					Path: "fields.cardrarity",
					Value: object.Fields.Cardrarity,
				},{
					Path: "fields.cardimageurl",
					Value: object.Fields.Cardimageurl,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getGamingcarddetailsList(subject *GAMINGCARDDETAILS) []*GAMINGCARDDETAILS {
	list := []*GAMINGCARDDETAILS{}
