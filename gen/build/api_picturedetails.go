...
// api-picturedetails
func (app *App) EntrypointPICTUREDETAILS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addPicturedetailsAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removePicturedetailsAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				{
					Path: "fields.seriesname",
					Value: object.Fields.Seriesname,
				},{
					Path: "fields.imagetitle",
					Value: object.Fields.Imagetitle,
				},{
					Path: "fields.pictureurl",
					Value: object.Fields.Pictureurl,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getPicturedetailsList(subject *PICTUREDETAILS) []*PICTUREDETAILS {
	list := []*PICTUREDETAILS{}
