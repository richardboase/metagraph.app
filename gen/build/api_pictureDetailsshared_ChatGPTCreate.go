...
func (app *App) picturedetailsChatGPTCreate(user *User, parent *PICTUREDETAILS, prompt string) error {
...
We want to create one or more of these data objects: Additional details for picture tokens

...for this parent object: ` + parentString + `

{

	// Name of the series  (THIS FIELD IS REQUIRED)
	seriesname (string)

	// Title of the image  (THIS FIELD IS REQUIRED)
	imagetitle (string)

	// URL or reference to the uploaded picture  (THIS FIELD IS REQUIRED)
	pictureurl (string)
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewPICTUREDETAILS(&parent.Meta, FieldsPICTUREDETAILS{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentPICTUREDETAILS(&parent.Meta, object); err != nil {
			return err
		}
