...
// api-street
func (app *App) EntrypointSTREET(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.GetDocument(app.App, id, street); err != nil {
	...
	case "POST":
		...
		case "update":
			...
			if err := street.Meta.SaveToFirestore(app.App, street); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
	...
	case "GET":
		...
		// return a specific street object by id
		case "object":

			cloudfunc.ServeJSON(w, street)
			return
	...
	case "DELETE":
		...
		_, err := street.Meta.Firestore(app.App).Delete(app.Context())
		if err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return
		}
...
func (app *App) getStreetList(subject *models.STREET) []*models.STREET {
	list := []*models.STREET{}
