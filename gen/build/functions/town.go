...
// api-town
func (app *App) EntrypointTOWN(w http.ResponseWriter, r *http.Request) {
	...
	town := &models.TOWN{}
	if err := utils.GetDocument(app.App, id, town); err != nil {
	...
	case "POST":
		...
		case "update":
			...
			if err := town.Meta.SaveToFirestore(app.App, town); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
	...
	case "GET":
		...
		// return a specific town object by id
		case "object":

			cloudfunc.ServeJSON(w, town)
			return
	...
	case "DELETE":
		...
		_, err := town.Meta.Firestore(app.App).Delete(app.Context())
		if err != nil {
			cloudfunc.HttpError(w, err, http.StatusInternalServerError)
			return
		}
...
func (app *App) getTownList(subject *models.TOWN) []*models.TOWN {
	list := []*models.TOWN{}
