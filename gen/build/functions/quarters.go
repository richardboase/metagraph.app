...
func (app *App) EntrypointQUARTERS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := models.FieldsQUARTER{}
			quarter := models.NewQUARTER(parent, fields)
	...
	case "GET":
		...
		// return a list of quarters in a specific parent
		case "list", "altlist":
			...
			q := parent.Firestore(app.App).Collection("quarters").OrderBy("Meta.Modified", firestore.Desc)
			...
			for {
				...
				if err := doc.DataTo(quarter); err != nil {
					log.Println(err)
					continue
				}
				list = append(list, quarter)
			}
