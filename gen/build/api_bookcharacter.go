...
// api-bookcharacter
func (app *App) EntrypointBOOKCHARACTER(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addBookcharacterAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeBookcharacterAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
...
func (app *App) getBookcharacterList(subject *BOOKCHARACTER) []*BOOKCHARACTER {
	list := []*BOOKCHARACTER{}
