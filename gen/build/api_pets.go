...
// api-pets
func (app *App) EntrypointPETS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			fields := FieldsPET{}
			object := user.NewPET(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentPET(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of pets
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "pets"),
			}
		...
		case "list":
			...
			app.petLists(w, user, nil, mode, limit)
