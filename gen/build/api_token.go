...
// api-token
func (app *App) EntrypointTOKEN(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addTokenAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeTokenAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "prompt":
			...
			reply, err := app.tokenChatGPTPrompt(user, object, prompt)
			if err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				{
					Path: "fields.tokentype",
					Value: object.Fields.Tokentype,
				},{
					Path: "fields.supply",
					Value: object.Fields.Supply,
				},{
					Path: "fields.hasdividend",
					Value: object.Fields.Hasdividend,
				},{
					Path: "fields.website",
					Value: object.Fields.Website,
				},{
					Path: "fields.twitter",
					Value: object.Fields.Twitter,
				},{
					Path: "fields.telegram",
					Value: object.Fields.Telegram,
				},{
					Path: "fields.liquidityaddress",
					Value: object.Fields.Liquidityaddress,
				},{
					Path: "fields.burnamount",
					Value: object.Fields.Burnamount,
				},{
					Path: "fields.mintlocation",
					Value: object.Fields.Mintlocation,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getTokenList(subject *TOKEN) []*TOKEN {
	list := []*TOKEN{}
