...
func (app *App) creatorChatGPTCreate(user *User, parent *CREATOR, prompt string) error {
...
We want to create one or more of these data objects: Information about the token creator

...for this parent object: ` + parentString + `

{

	// Name of the creator  (THIS FIELD IS REQUIRED)
	name (string)
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewCREATOR(&parent.Meta, FieldsCREATOR{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentCREATOR(&parent.Meta, object); err != nil {
			return err
		}
