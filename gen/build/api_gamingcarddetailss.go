...
// api-gamingcarddetailss
func (app *App) EntrypointGAMINGCARDDETAILSS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "ai":
			...
			case "openai":
				...
				case "prompt":
					...
					reply, err := app.gamingcarddetailsChatGPTPrompt(user, object, prompt)
					if err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "create":
					...
					if err := app.gamingcarddetailsChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.gamingcarddetailsChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.gamingcarddetailsVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
		...
		case "init":
			...
			if err := app.CreateDocumentGAMINGCARDDETAILS(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadGAMINGCARDDETAILS(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of gamingcarddetailss
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "gamingcarddetailss"),
			}
		...
		case "list":
			...
			app.gamingcarddetailsLists(w, user, parent, mode, limit)
