...
func (app *App) bookcharacterLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("bookcharacters")
	} else {
		collection = app.Firestore().Collection("bookcharacters")
	}
