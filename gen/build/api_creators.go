...
// api-creators
func (app *App) EntrypointCREATORS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			object := user.NewCREATOR(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentCREATOR(nil, object); err != nil {
	...
	case "GET":
		...
		// return the total amount of creators
		case "count":

			data := map[string]int{
				"count": FirestoreCount(app.App, "creators"),
			}
		...
		case "list":
			...
			app.creatorLists(w, user, nil, mode, limit)
