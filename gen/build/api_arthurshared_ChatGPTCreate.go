...
func (app *App) arthurChatGPTCreate(user *User, parent *ARTHUR, prompt string) error {
...
We want to create one or more of these data objects: arthurs space
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewARTHUR(&parent.Meta, FieldsARTHUR{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentARTHUR(&parent.Meta, object); err != nil {
			return err
		}
