...
func (app *App) donationChatGPTCreate(user *User, parent *DONATION, prompt string) error {
...
We want to create one or more of these data objects: record of a donation made to the rescue center
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewDONATION(&parent.Meta, FieldsDONATION{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentDONATION(&parent.Meta, object); err != nil {
			return err
		}
