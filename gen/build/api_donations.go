...
// api-donations
func (app *App) EntrypointDONATIONS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "init":
			...
			object := user.NewDONATION(nil, fields)
			if !object.ValidateInput(w, m) {
				return
			}

			// reuse document init create code
			if err := app.CreateDocumentDONATION(nil, object); err != nil {
	...
	case "GET":
		...
		case "count":
			...
			data := map[string]int{
				"count": FirestoreCount(app.App, "donations"),
			}
		...
		case "list":
			...
			app.donationLists(w, user, nil, mode, limit)
