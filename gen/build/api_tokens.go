...
// api-tokens
func (app *App) EntrypointTOKENS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "ai":
			...
			case "openai":
				...
				case "create":
					...
					if err := app.tokenChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.tokenChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.tokenVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
		...
		case "init":
			...
			if err := app.CreateDocumentTOKEN(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadTOKEN(w, r, parent, user)
			return
	...
	case "GET":
		...
		case "count":
			...
			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "tokens"),
			}
		...
		case "list":
			...
			app.tokenLists(w, user, parent, mode, limit)
