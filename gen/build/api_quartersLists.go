...
func (app *App) quarterLists(w http.ResponseWriter, user *User, parent *Internals, mode string, limit int) {

	var collection *firestore.CollectionRef
	if parent != nil {
		collection = parent.Firestore(app.App).Collection("quarters")
	} else {
		collection = app.Firestore().Collection("quarters")
	}
