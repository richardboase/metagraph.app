...
// api-jellys
func (app *App) EntrypointJELLYS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "openai":
			...
			case "prompt":
				...
				reply, err := app.jellyChatGPTPrompt(user, object, prompt)
				if err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "create":
				...
				if err := app.jellyChatGPTCreate(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			...
			case "modify":
				...
				if err := app.jellyChatGPTModify(user, object, prompt); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
		...
		case "init":
			...
			if err := app.CreateDocumentJELLY(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadJELLY(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of jellys
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "jellys"),
			}
		...
		case "list":
			...
			app.jellyLists(w, user, parent, mode, limit)
