...
// api-jellyname
func (app *App) EntrypointJELLYNAME(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addJellynameAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removeJellynameAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
...
func (app *App) getJellynameList(subject *JELLYNAME) []*JELLYNAME {
	list := []*JELLYNAME{}
