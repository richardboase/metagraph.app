...
func (app *App) gamingcarddetailsChatGPTCreate(user *User, parent *GAMINGCARDDETAILS, prompt string) error {
...
We want to create one or more of these data objects: Additional details for gaming card tokens

...for this parent object: ` + parentString + `

{

	// Title of the game  (THIS FIELD IS REQUIRED)
	gametitle (string)

	// Type of the card  (THIS FIELD IS REQUIRED)
	cardtype (string)

	// Rarity of the card (Common, Rare, Legendary)  (THIS FIELD IS REQUIRED)
	cardrarity (string)

	// URL or reference to the uploaded card image  (THIS FIELD IS REQUIRED)
	cardimageurl (string)
...
The response should be a raw JSON array with one or more objects, based on the user prompt: `
	...
	for _, r := range newResults {
		...
		object := user.NewGAMINGCARDDETAILS(&parent.Meta, FieldsGAMINGCARDDETAILS{})
		if err := object.ValidateObject(result); err != nil {
			return err
		}
		if err := app.CreateDocumentGAMINGCARDDETAILS(&parent.Meta, object); err != nil {
			return err
		}
