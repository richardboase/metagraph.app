...
// api-pet
func (app *App) EntrypointPET(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "admin":
			...
			switch mode {
			case "add":
				if err := app.addPetAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			case "remove":
				if err := app.removePetAdmin(object, admin); err != nil {
					cloudfunc.HttpError(w, err, http.StatusInternalServerError)
					return
				}
			default:
		...
		case "prompt":
			...
			reply, err := app.petChatGPTPrompt(user, object, prompt)
			if err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return
			}
		...
		case "update":
			...
			updates := []firestore.Update{
				...
				},
				{
					...
					Path: "fields.breed",
					Value: object.Fields.Breed,
				},{
					Path: "fields.age",
					Value: object.Fields.Age,
				},{
					Path: "fields.medicalhistory",
					Value: object.Fields.Medicalhistory,
				},{
					Path: "fields.adoptionstatus",
					Value: object.Fields.Adoptionstatus,
				},
			}
			for _, update := range updates {
				println(update.Path, update.Value)
...
func (app *App) getPetList(subject *PET) []*PET {
	list := []*PET{}
