...
// api-musicdetailss
func (app *App) EntrypointMUSICDETAILSS(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "ai":
			...
			case "openai":
				...
				case "prompt":
					...
					reply, err := app.musicdetailsChatGPTPrompt(user, object, prompt)
					if err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "create":
					...
					if err := app.musicdetailsChatGPTCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
				...
				case "modify":
					...
					if err := app.musicdetailsChatGPTModify(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
			...
			case "vertex":
				...
				case "create":
					...
					if err := app.musicdetailsVertexCreate(user, object, prompt); err != nil {
						cloudfunc.HttpError(w, err, http.StatusInternalServerError)
						return
					}
		...
		case "init":
			...
			if err := app.CreateDocumentMUSICDETAILS(parent, object); err != nil {
				cloudfunc.HttpError(w, err, http.StatusInternalServerError)
				return				
			}
		...
		case "upload":
			...
			app.UploadMUSICDETAILS(w, r, parent, user)
			return
	...
	case "GET":
		...
		// return the total amount of musicdetailss
		case "count":

			data := map[string]int{
				"count": parent.FirestoreCount(app.App, "musicdetailss"),
			}
		...
		case "list":
			...
			app.musicdetailsLists(w, user, parent, mode, limit)
